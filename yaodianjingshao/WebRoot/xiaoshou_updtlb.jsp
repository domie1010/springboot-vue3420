	...
	<body>
			<form action="updateXiaoshou.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="20" colspan="2" background="images/table_header.gif">设置出库<input type="hidden" name="id" value="${xiaoshou.id}" /></td>
						</tr>
		...
		<tr style='display:none'><td width="200">客户：</td><td><select name='kehu' id='kehu' style=' height:19px; border:solid 1px #000000; color:#666666'><%=connDbBean.hsggetoption("kehuxinxi","kehu")%></select></td></tr>
		<tr style='display:none'><td width="200">出库数量：</td><td><input name='xiaoshoushuliang' type='text' id='xiaoshoushuliang' value='' onblur='checkform()' style='border:solid 1px #000000; color:#666666' />&nbsp;<label id='clabelxiaoshoushuliang' />必需数字型</td></tr>
		<tr style='display:none'><td width="200">出库时间：</td><td><input name='xiaoshoushijian' type='text' id='xiaoshoushijian' value='<%=connDbBean.getdate()%>' onblur='' readonly='readonly' onClick="WdatePicker({'dateFmt':'yyyy-MM-dd'})" style='width:100px; height:16px; border:solid 1px #000000; color:#666666'/></td></tr>
		<tr style='display:none'><td width="200">出库员：</td><td><input name='xiaoshouyuan' type='text' id='xiaoshouyuan' onblur='' style='border:solid 1px #000000; color:#666666' value='<%=(String)request.getSession().getAttribute("username")%>' readonly="readonly" /></td></tr>
