...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addGongyingshangxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="20" colspan="2" background="images/table_header.gif">添加供应商信息</td>
						</tr>
...
function hsgcheck() {
		...
		if(gongyingshang==""||(gongyingshang.length<6||gongyingshang.length>12)){
			 $("#clabelgongyingshang").html("<font color=red>供应商不能为空且长度在3～12位之间！</font>");
			 $("input[id=gongyingshang]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelgongyingshang").html("");
			$.ajax({
				url : "quchongGongyingshangxinxi.do",
				type : "post",
				data : "gongyingshang=" + gongyingshang,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelgongyingshang").html("<font color=red>供应商已存在，请更换！</font>");
					$("input[id=gongyingshang]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelgongyingshang").html("系统异常，请检查错误！");
					$("input[id=gongyingshang]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
