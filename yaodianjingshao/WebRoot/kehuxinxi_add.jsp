...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addKehuxinxi.do" name="form1" method="post">
...
function hsgcheck() {
		var kehu = $("#kehu").val();
		if(kehu==""||(kehu.length<6||kehu.length>12)){
			 $("#clabelkehu").html("<font color=red>客户不能为空且长度在3～12位之间！</font>");
			 $("input[id=kehu]").focus();
		...
		else
		{
			$("#clabelkehu").html("");
			$.ajax({
				url : "quchongKehuxinxi.do",
				type : "post",
				data : "kehu=" + kehu,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelkehu").html("<font color=red>客户已存在，请更换！</font>");
					$("input[id=kehu]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
